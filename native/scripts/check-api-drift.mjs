...
import { existsSync, appendFileSync, writeFileSync, readFileSync, mkdtempSync } from "fs";
import { tmpdir } from "os";
import { join } from "path";
import { extractExportsFromFile } from "./api-surface.mjs";

const SNAPSHOT = ".upstream-api-snapshot.json";
const UPSTREAM_INDEX_LOCAL = "../versions/18/src/index.ts";
const UPSTREAM_INDEX_URL =
  "https://raw.githubusercontent.com/constructive-io/libpg-query-node/main/versions/18/src/index.ts";
const NATIVE_INDEX = "src/index.ts";

const update = process.argv.includes("--update");

async function resolveUpstreamIndex() {
  if (existsSync(UPSTREAM_INDEX_LOCAL)) {
    return UPSTREAM_INDEX_LOCAL;
  }
  ...
  const dir = mkdtempSync(join(tmpdir(), "libpg-query-upstream-"));
  const path = join(dir, "index.ts");
  writeFileSync(path, await res.text());
  return path;
}

const upstreamPath = await resolveUpstreamIndex();
const upstream = extractExportsFromFile(upstreamPath);
const native = extractExportsFromFile(NATIVE_INDEX);

if (upstream.unclassified.length) {
  console.log(
    `::warning::${upstream.unclassified.length} unparsed export line(s) upstream; drift detection may be incomplete.`
  );
}

const upstreamSource = existsSync(UPSTREAM_INDEX_LOCAL)
  ? UPSTREAM_INDEX_LOCAL
  : UPSTREAM_INDEX_URL;

if (update) {
  writeFileSync(
    SNAPSHOT,
    JSON.stringify({ source: upstreamSource, exports: upstream.names }, null, 2) + "\n"
  );
  console.log(`Snapshot updated: ${upstream.names.length} exports from ${upstreamSource}`);
  process.exit(0);
}
...
if (missing.length) {
  lines.push("");
  lines.push(
    `<details><summary>Upstream exports not present in the native API (${missing.length})</summary>`
  );
  lines.push("");
