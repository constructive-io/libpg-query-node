...
function publishVersion(version, bumpType) {
  const packageDir = path.join(__dirname, '..', 'types', version);
  
  console.log(`\n📦 Publishing @libpg-query/types${version}...`);
  ...
  try {
    ...
    execSync(`git commit -m "release: bump @libpg-query/types${version} to ${newVersion}"`, { stdio: 'inherit' });
    ...
    execSync('pnpm prepare:types', { stdio: 'inherit' });
    
    // Publish
    console.log(`  - Publishing to npm with tag pg${version}...`);
    execSync(`pnpm publish --tag pg${version} --no-git-checks`, { stdio: 'inherit' });
    
    console.log(`✅ Successfully published @pgsql/types@${newVersion} with tag pg${version}`);
...
async function main() {
  console.log('🚀 Types Package Publisher\n');
  ...
  if (versions.includes('17')) {
    ...
    if (promoteAnswer === 'yes' || promoteAnswer === 'y') {
      try {
        ...
        execSync('npm dist-tag add @pgsql/types@pg17 latest', { stdio: 'inherit' });
